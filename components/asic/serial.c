...
esp_err_t SERIAL_init(void)
{
    ...
    return uart_driver_install(UART_NUM_1, BUF_SIZE * 2, 0 /* BUF_SIZE * 2 */, 0, NULL, 0);
}
...
int SERIAL_send(const uint8_t *data, int len, bool debug)
{
    if (debug)
    {
        printf("tx: ");
        prettyHex((const unsigned char *)data, len);
        printf("\n");
    }
...
int SERIAL_rx(uint8_t* const buf, const size_t size, const TickType_t maxWait)
{
    int bytes_read = uart_read_bytes(UART_NUM_1, buf, size, maxWait);
